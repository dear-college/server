...
instance ToJSON Header where
  toJSON (Header jwpType algorithm d) =
    object
      [ "typ" .= jwpType,
        "alg" .= algorithm,
        "dif" .= d
...
instance ToJSON Payload where
  toJSON (Payload expiry subscriber audience) = object ["exp" .= expiry, "sub" .= subscriber, "aud" .= audience]
...
difficulty b =
  let hashed = hashWith SHA256 b
      bs = Data.ByteArray.convert hashed
   in countZeros bs
...
verify jwp d s = do
  ...
  if Just s /= maybeSubject
    then pure $ Left "Subject mismatch"
    else
      if claimedDifficulty < d
        then pure $ Left "Claimed difficulty is too low"
        else
          if actualDifficulty < claimedDifficulty
            then pure $ Left "Actual difficulty is too low"
            else
              if not unexpired
