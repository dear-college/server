...
import AppM (AppM, HasCookieSettings(..), HasJwtSettings(..), HasOidcEnvironment(..), HasConfiguration (..), HasOidcEnvironment (..), MonadDB (..), getConfiguration, getPool)

import OIDC.Types (genRandomBS, OIDCConf(..), OIDCEnv(..))
import Web.OIDC.Client.Types (Code, State, SessionStore(..))
...
type API = "login" :> ( -- redirect User to the OpenID Provider
  Get '[JSON] NoContent
...
instance FromJSON OtherClaims where
  parseJSON = AeT.withObject "OtherClaims" $ \v -> OtherClaims
      <$> v .: "given_name"
      <*> v .: "family_name"
      <*> v .: "picture"
      <*> v .: "email"
